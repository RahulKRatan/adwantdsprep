...
public class InsertDeleteGetRandomO1 {
    ...
    public boolean remove(int val) {
        ...
        if (loc < nums.size() - 1) { // not the last one than swap the last one with this val
            ...
            nums.set(loc,lastone); // replace removing element with last element so we can delete last element
            locs.put(lastone,loc); // replace value of the above replaced item with correct one.
        }
        locs.remove(val);
